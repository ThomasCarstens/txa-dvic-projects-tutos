...
from cflib.positioning.motion_commander import motion_commander

URI = 'radio://0/80/2M' #TO BE MATCHED
...
if __name__ == '__main__':
    sflib.crtp.init_drivers(enable_debug_driver=False)

    with SyncCrazyflie(URI) as scf:
        #We take off when the commander is created
        mc = MotionCommander(scf)
        drone = KeyboardDrone(mc)
        with keyboard.Listener(on_press=drone.on_pres, on_release=drone.on_release) as listener:
            listener.join()
