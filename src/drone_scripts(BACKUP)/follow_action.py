...
class ArenaFlyer():
    ...
    def __init__(self):
        ...
        self.cf3_subscriber = rospy.Subscriber('/tf', tf2_msgs.msg.TFMessage, self.cf3_callback)
    ...
    def execute_cb(self, goal):
        ...
        for cf in self.allcfs.crazyflies:
            if cf.id == 2:
                cf.goTo(np.array[self.cf3_pose.x+0.5, self.cf3_pose.y+0.5, self.cf3_pose.z+0.5])
    ...
    def follow_transition(self):
        distance_tolerance = 1
        while self.euclidean_distance(self.cf3_pose) >= distance_tolerance:
            self.success = False

        if self.euclidean_distance(self.cf3_pose) < distance_tolerance:
            self.success = True
