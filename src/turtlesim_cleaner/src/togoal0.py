...
class FibonacciAction(object):
    ...
    def execute_cb(self, goal):
        ...
        #ideally these two are: self.turtle2_pose, self.turtle1_pose
        goal_pose = Point()
        ...
        turtle1_pose= Pose()
        if id == 1:
            turtle_pose = self.turtle1_pose
        if id == 2:
            turtle_pose = self.turtle2_pose
        ...
        while self.success == False:
            ...
            try:
                ...
                self.move2goal(goal_pose, turtle_pose)
    ...
    def euclidean_distance(self, goal_pose, turtle_pose):
        """Euclidean distance between current pose and the goal."""
        euclidean_distance= sqrt(pow((goal_pose.x - turtle_pose.x), 2) + pow((goal_pose.y - turtle_pose.y), 2))
	    #print("distance is", round(euclidean_distance, 4), "between", goal_pose.x, "and", self.turtle1_pose.x )
        return euclidean_distance


    def linear_vel(self, goal_pose, turtle_pose, constant=0.1):
        """See video: https://www.youtube.com/watch?v=Qh15Nol5htM."""
        return constant * self.euclidean_distance(goal_pose, turtle_pose)

    def steering_angle(self, goal_pose, turtle_pose):
        """See video: https://www.youtube.com/watch?v=Qh15Nol5htM."""
        return atan2(goal_pose.y - turtle_pose.y, goal_pose.x - turtle_pose.x)

    def angular_vel(self, goal_pose, turtle_pose, constant=3):
        """See video: https://www.youtube.com/watch?v=Qh15Nol5htM."""
        return constant * (self.steering_angle(goal_pose, turtle_pose) - turtle_pose.theta)

    def move2goal(self, goal_pose, turtle_pose):
        ...
        while (turtle_pose.x == 0.0):
            print ("sleepn")
            rospy.wait_for_message("/turtle2/pose", Pose, timeout=None)
            #self.rate.sleep()

        if turtle_pose.x != 0.0 and turtle_pose.y != 0.0:


            while self.euclidean_distance(goal_pose, turtle_pose) >= distance_tolerance:
                self.success = False
                print(turtle_pose)
                ...
                vel_msg.linear.x = self.linear_vel(goal_pose, turtle_pose)
                vel_msg.linear.y = 0
                vel_msg.linear.z = 0
                ...
                vel_msg.angular.x = 0
                vel_msg.angular.y = 0
                vel_msg.angular.z = self.angular_vel(goal_pose, turtle_pose)
            ...
            if self.euclidean_distance(goal_pose, turtle_pose) < distance_tolerance:
                vel_msg.linear.x = 0
                vel_msg.angular.z = 0
